import { eventMatches, on } from '../misc';
  ...
  class InitComponentByEvent extends ToMix {
    ...
    static init(target = document, options = {}) {
      ...
      if (target.nodeType === Node.ELEMENT_NODE && target.matches(effectiveOptions.selectorInit)) {
        ...
        // To work around non-bubbling `focus` event,
        // use `focusin` event instead of it's available, and "capture mode" otherwise
        const hasFocusin
          = 'onfocusin' in (target.nodeType === Node.ELEMENT_NODE ? target.ownerDocument : target).defaultView;
